...
\title{Comparison of Genetic Algorithm Training Methods As Applied To Tic-Tac-Toe} % Put your proper title here
\dayofmonth{4} % Depends on prof
\date{March 2019} % Also depends on prof

\tptitle{
	Comparison Of Genetic Algorithm Training Methods\\
	As Applied To Tic-Tac-Toe
} % Format this to make it the title an inverted pyramid
\addAuthor{Vash Patrick B. Ancheta} % Names in alphabetical order (First M.I. Family Name)
\addAuthor{Diego Sulayman R. Pascua}
\addAuthor{Resh Vnzi S. Togue\~no}
\adviser{Kaye Melina Natividad B. Alamag} % Name of adviser
...
\renewcommand{\makeabstract}[1]{
	...
	% Uncomment this to add indent to text
	% \indent
	#1
